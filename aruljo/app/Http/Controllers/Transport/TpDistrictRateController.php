...
class TpDistrictRateController extends Controller
{
    ...
    {
        $rates = TpDistrictRate::with(['location', 'office'])->latest()->get();
        ...
        return view('transport.rates.index', compact('rates', 'states', 'offices'));
    }
    ...
    {
        $offices = TpOffice::orderBy('name')->get();
        return view('transport.rates.create', compact('offices'));
    }
    ...
    {
        $rate = \App\Models\Transport\TpDistrictRate::with('location')->findOrFail($id);
    ...
    {
        $rate = \App\Models\Transport\TpDistrictRate::findOrFail($id);
        ...
        if ($request->ajax()) {
            return response()->json([
                ...
                'data' => $rate->load('office', 'location'),
            ]);
        }
    ...
    public function getDistricts(Request $request)
    {
        ...
        $districts = \App\Models\DistancePincode::getDistrictsByState($state);
    ...
    public function getPlaces(Request $request)
    {
        ...
        $places = \App\Models\DistancePincode::getPlacesByStateAndDistrict($state, $district);
